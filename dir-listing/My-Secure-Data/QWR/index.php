<?php
	if(!isset($_COOKIE['specified_uid'])) {
	...
	else
	{
		...
		// Chang user1 --> to your desigre username from line 10
		if ($login->username !== "user3")
		{
			echo "You are not authorize to see this page with the credentials you entered !";
			die();
		}
		else
		{
...
These files must be located within the system_files directory.
*/
$filetypes = array (
				'png' => 'jpg.gif',
				'jpeg' => 'jpg.gif',
...
<title>Your Secure Directory</title>
<link rel="stylesheet" type="text/css" href="<?php echo $includeurl; ?>system_files/styles.css" />
<?php
if($showthumbnails) {
?>
...
<!--
function o(n, i) {
	document.images['thumb'+n].src = '<?php echo $includeurl; ?>system_files/i.php?f='+i<?php if($memorylimit!==false) echo "+'&ml=".$memorylimit."'"; ?>;

}

function f(n) {
	document.images['thumb'+n].src = 'system_files/trans.gif';
}
//-->
</script>
<?php
...
<div id="container">
	<?php 
		if ($auth_user == TRUE)
		{
			echo "<p><strong>Welcome "; 
			echo $login->username;
			echo "</strong> <br /> <span style='font-size: 12px; color:red;'>DO NOT FORGET to </span><a href='../../secure_logout.php'> Logout</a></p>";
		}
	?>
  <h1>Here are your files and folder<?php // echo str_replace('\\', '', dirname(strip_tags($_SERVER['PHP_SELF']))).'/'.$leadon;?></h1>
  <div id="breadcrumbs">  You are here: <a href="<?php echo strip_tags($_SERVER['PHP_SELF']);?>">Home</a> 
  <?php
  ...
  <div id="listingcontainer">
	...
	if($dirok) {
	?>
	<div><a href="<?php echo strip_tags($_SERVER['PHP_SELF']).'?dir='.urlencode($dotdotdir);?>" class="<?php echo $class;?>"><img src="<?php echo $includeurl; ?>system_files/dirup.png" alt="Folder" /><strong>..</strong> <em>&nbsp;</em>&nbsp;</a></div>
	<?php
		if($class=='b') $class='w';
		else $class = 'b';
	}
	...
	for($i=0;$i<$arsize;$i++) {
	?>
	<div><a href="<?php echo strip_tags($_SERVER['PHP_SELF']).'?dir='.urlencode(str_replace($startdir,'',$leadon).$dirs[$i]);?>" class="<?php echo $class;?>"><img src="<?php echo $includeurl; ?>system_files/folder.png" alt="<?php echo $dirs[$i];?>" /><strong><?php echo $dirs[$i];?></strong> <em>-</em> <?php echo date ("M d Y h:i:s A", filemtime($includeurl.$leadon.$dirs[$i]));?></a></div>
	<?php
		if($class=='b') $class='w';
		else $class = 'b';	
	}
	...
	for($i=0;$i<$arsize;$i++) {
		...
		if($showthumbnails && in_array($ext, $supportedimages)) {
			$thumb = '<span><img src="system_files/trans.gif" alt="'.$files[$i].'" name="thumb'.$i.'" /></span>';
	...
	?>
	<div><a href="<?php echo $fileurl;?>" class="<?php echo $class;?>"<?php echo $thumb2;?>><img src="<?php echo $includeurl; ?>system_files/<?php echo $icon;?>" alt="<?php echo $files[$i];?>" /><strong><?php echo $filename;?></strong> <em><?php echo round(filesize($includeurl.$leadon.$files[$i])/1024);?>KB</em> <?php echo date ("M d Y h:i:s A", filemtime($includeurl.$leadon.$files[$i]));?><?php echo $thumb;?></a></div>
	<?php
		if($class=='b') $class='w';
		else $class = 'b';	
	}	
